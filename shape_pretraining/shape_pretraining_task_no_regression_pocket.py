...
@register_task
class ShapePretrainingTaskNoRegressionPocket(BaseTask):
    def __init__(self,
                 grid_resolution=1.0,
                 max_dist_stamp=3.0,
                 max_dist=10.0,
    ...
    def _collate(self, samples):
        ...
        if not self._infering or (self._infering and self._teacher_force_inference):
            ...
            raise Exception('This task is for evaluation only right now!!')
            ...
            if self._for_protein_decode:
                nof = torch.tensor(np.array(nof), dtype=torch.float)
            ...
            if self._for_protein_decode:
                net_input = {
                    'encoder': (shape_patches,),
                    'decoder': ((input_frag_idx, input_frag_trans, input_frag_r_mat, False, nof),),
                }
            else:
                net_input = {
                    'encoder': (shape_patches,),
